...
const drawToneCurve = () => {
  ...
  // 设置样式
  ctx.strokeStyle = getComputedStyle(document.documentElement).getPropertyValue('--c-brand').trim() || '#3A6B8C'
  ctx.lineWidth = 3
  ctx.lineCap = 'round'
  ctx.lineJoin = 'round'
  
  // 绘制网格
  ctx.strokeStyle = 'rgba(27, 58, 92, 0.1)'
  ctx.lineWidth = 1
  for (let i = 1; i < 5; i++) {
    const y = (height / 5) * i
    ctx.beginPath()
  ...
  props.tones.forEach((tone, index) => {
    ...
    // 绘制点
    ctx.fillStyle = getComputedStyle(document.documentElement).getPropertyValue('--c-brand').trim() || '#3A6B8C'
    ctx.beginPath()
    ...
    // 绘制标签
    ctx.fillStyle = getComputedStyle(document.documentElement).getPropertyValue('--c-text-70').trim() || 'rgba(27,58,92,0.7)'
