...
// 类型 → 精确跳转参数
// 词典支持 type 过滤 + focus 自动聚焦；答题支持 start 自动开始
function preciseLinkForType(type, action = 'view') {
  if (type === 'quiz') return '/quiz?start=1'
  if (type === 'song') return '/songs'
  if (type === 'dictionary') return '/dictionary?type=word&focus=1'
  if (type === 'phrase') return '/dictionary?type=phrase&focus=1'
  if (type === 'proverb') return '/dictionary?type=proverb&focus=1'
  // 兜底：通用路由 + 聚焦
...
export function generateSuggestions(stats = {}) {
  ...
  if (totalCount === 0) {
    return [buildSuggestion({
      ...
      link: '/dictionary?type=word&focus=1',
      icon: '🌱',
      priority: 100,
      key: 'first-word'
    })]
  ...
  if (todayCount === 0) {
    const link = weekend ? '/songs' : '/dictionary?type=word&focus=1'
  ...
  if (stage === 'beginner' && !todayLearnedTypes.has('phrase')) {
    ...
    suggestions.push(buildSuggestion({
      text: '词汇初有积累，试试短句更地道',
      link: '/dictionary?type=phrase&focus=1',
      icon: '📝',
      priority: 76,
      key: 'stage-phrase'
    }))
    ...
    suggestions.push(buildSuggestion({
      ...
      link: '/dictionary?type=proverb&focus=1',
      icon: '📜',
      priority: 76,
      key: 'stage-proverb'
    }))
