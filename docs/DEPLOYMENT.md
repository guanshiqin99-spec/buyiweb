...
> ⚠️ **当前生产实际部署方案以根目录 [`部署方案.md`](../部署方案.md) 为准**（阿里云北京 ECS + Cloudflare Named Tunnel + Cloudflare Pages + 微信云函数代理）。
> 本文档保留 Docker Compose / Nginx / PM2 等通用部署知识作为备用方案与运维参考，其中：
> - 第一章「部署方案对比」中的 **Vercel 方案已废弃下线**（2026-07-23 通过 Vercel CLI 删除 `backend` 与 `buyi-dictionary-vue` 两个项目，仓库中 `backend/vercel.json` 已删除），勿再使用
> - 第五章小程序部署的「服务器域名 HTTPS」要求**已被云函数代理方案替代**：小程序通过 `cloudfunctions/apiProxy` 直连后端 IP `http://39.96.81.132:80/api`，无需在微信公众平台配置合法域名
> - 容器化、Nginx 配置、回滚流程、运维命令等内容仍具备通用参考价值
...
| 方案 | 适用场景 | 优点 | 缺点 |
|------|---------|------|------|
| **Cloudflare + ECS（当前生产）** | 当前实际生产 | 稳定 HTTPS、CDN、免费、国内低延迟 | 配置项较多，详见 [`部署方案.md`](../部署方案.md) |
| Docker Compose | 中小规模、快速上线 | 一键启动、环境隔离 | 资源占用略高 |
| Nginx + PM2 + MySQL | 传统生产部署 | 性能最佳、运维成熟 | 配置繁琐 |
| 阿里云 / 腾讯云 ECS | 自建服务器 | 完全可控 | 需自行运维 |
| ~~Vercel + 云数据库~~ | ~~Serverless~~ | ~~极简部署~~ | **已废弃下线（2026-07-23），勿再使用** |
