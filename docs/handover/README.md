...
> 生成时间：2026-07-23，最后更新：2026-08-02。本文件夹是一次全库核查（前端、后端、部署配置、冗余、安全）的产出。

| 文件 | 内容 |
|------|------|
| [01-冗余文件清理清单.md](01-冗余文件清理清单.md) | 可删/需确认/必须保留三级清单，含 git 追踪状态与执行顺序 |
| [02-重大漏洞与修复清单.md](02-重大漏洞与修复清单.md) | 按严重度排序的漏洞清单（含已实测的线上 API 断裂、密码泄露） |

功能优化的完整方案见 [../决赛冲刺优化方案.md](../决赛冲刺优化方案.md)（已更新到 v4，以 v4 进度摘要为准）。
