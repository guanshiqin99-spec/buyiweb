...
| `切换RAG开关.bat` | 切换 AI 导览员的 RAG 检索增强开关（仅本地） |

> **RAG 开关说明**：AI 导览员默认开启 RAG（回答前先检索词典数据库增强）。双击 `切换RAG开关.bat` 可在开/关之间切换，切换后需重启本地后端（重新运行 `启动后端.bat`）生效。线上环境保持默认开启，不动此开关。用途：对比演示 RAG 开/关两种模式下的回答质量差异（关闭时 AI 直接调用大模型，答案可能与词典库不一致）。
