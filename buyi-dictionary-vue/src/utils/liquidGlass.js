...
var canBind = typeof window !== 'undefined' && window.matchMedia
...
// 路由切换会卸载部分卡片；先剔除失联节点，避免持续持有旧页面引用。
function pruneNodes() {
  glasses = glasses.filter(function (el) { return el && el.isConnected })
  glowOnly = glowOnly.filter(function (el) { return el && el.isConnected })
}
...
function collect() {
  pruneNodes()
  var liquidNodes = document.querySelectorAll('.liquid-glass, .liquid-glass-content, .liquid-glass-hero, .liquid-glass-quiet')
  for (var i = 0; i < liquidNodes.length; i++) {
    var el = liquidNodes[i]
    if (el.closest('[data-motion-surface="tool"]')) continue
  ...
  if (canBind) {
    ...
    for (var j = 0; j < glowNodes.length; j++) {
      var gel = glowNodes[j]
      if (gel.classList.contains('liquid-glass') || gel.classList.contains('liquid-glass-content') || gel.classList.contains('liquid-glass-hero') || gel.classList.contains('liquid-glass-quiet')) continue
...
export function initLiquidGlass() {
  ...
  if (!reduceMotion) {
    scrollHandler = function () {
      ...
      lastScrollY = y
      scrollShift = clamp(scrollShift + vel * 0.03, -8, 8)
      applyScrollShift()
      maybeAdaptive()
