  ...
  <main id="main" class="quiz-page" data-motion-surface="tool" data-tool-page="">
    ...
    <section v-else-if="phase === 'question' && currentQuestion" class="quiz-question" aria-live="polite">
      ...
      <aside v-if="isAnswered" class="quiz-feedback liquid-glass-quiet" :class="{ 'quiz-feedback--right': selectedOption === currentQuestion.answer }">
        ...
        <SourceBadge
          class="quiz-feedback__source"
          :source="currentQuestion.source"
          :source-url="currentQuestion.source === '中国非物质文化遗产网' ? 'https://www.ihchina.cn/' : ''"
          :verified="currentQuestion.source === '中国非物质文化遗产网'"
        />
...
/* 固定背景图层：保留原图，仅在文字侧叠加渐变蒙层以提升对比度。 */
.quiz-page__bg { position: fixed; inset: -10%; z-index: -2; will-change: transform; }
/* 暗色渐变蒙层：左侧与底部加暗，保证文字区背景足够深。 */
.quiz-page__scrim { position: fixed; inset: 0; z-index: -1; pointer-events: none; background: var(--grad-hero-bottom), var(--grad-hero-scrim); }
/* 背景图上的无阴影文字补一层近距投影，确保亮区也能看清。 */
.quiz-intro > p, .quiz-question__type, .quiz-result > p, .quiz-question header, .quiz-result h1, .quiz-result > span { text-shadow: 0 1px 2px var(--c-shadow-40); }
