...
Page({
  ...
  async loadItems(page = 1) {
    ...
    try {
      const payload = await contentApi.listByType('proverb', page, 20);
      let nextItems = mapContentList(payload.items || [], 'proverb');
      ...
      this.setData({ items: page === 1 ? [] : this.data.items, loading: false, loadingMore: false, errorText: page === 1 ? '谚语加载失败' : '' });
      if (page > 1) wx.showToast({ title: '更多谚语加载失败', icon: 'none' });
    }
  },
