...
### Cultural/Themed Design Override

When a project has a pre-existing cultural or themed design identity (e.g., Chinese blue-and-white porcelain / 青花瓷, ink painting / 山水墨韵), the aesthetic direction must respect and enhance that identity rather than replacing it.

**Aesthetic Direction Override for Cultural Projects:**
- Cultural/Heritage (青花瓷 / 山水墨韵 / 民族风格) -- Use traditional art as primary aesthetic driver
- The project's existing color palette and design tokens take priority over the generic aesthetic options above
- Typography should use culturally appropriate fonts (e.g., Noto Serif SC for Chinese cultural projects)
- Decorative elements should reference traditional patterns (porcelain motifs, ink wash textures, ethnic ornaments)
- Layout asymmetry should follow the visual rhythm of traditional composition, not Western grid-breaking

**Brand Escape Hatch applies fully here** -- document the cultural design constraints explicitly in the Design Specification and treat them as higher-priority than the generic rules.

**Cultural design anti-patterns to avoid:**
- Slapping traditional patterns as background without integrating them into the layout
- Using generic "modern" rounded cards that clash with the cultural aesthetic
- Over-simplifying traditional motifs into flat, lifeless icons
- Ignoring the dark/light theme duality inherent in many cultural design systems
...
- **Recommended Animation Libraries (Vue 3 ecosystem)**:
  - **GSAP** (`greensock/GSAP`) -- Professional-grade, framework-agnostic. Use for hero entrance animations, scroll-triggered parallax, complex timelines. Best for high-impact showcase moments.
  - **@vueuse/motion** (`vueuse/motion`) -- Vue-native, declarative API. Use for simple component transitions, hover effects, reactive animations.
  - **Lenis** (`darkroom-engineering/lenis`) -- Buttery smooth scroll replacement. Use for culture showcase pages and immersive reading experiences.
  - **Anime.js** (`juliangarnier/anime`) -- Lightweight, framework-agnostic. Use for micro-interactions, number counters, SVG path animations.
- **Animation Strategy for Competition Projects**: Use GSAP for the hero/culture page (high impact), Vue transitions for route changes, CSS keyframes for micro-interactions. Focus animation effort on 2-3 "wow moments" rather than spreading evenly.
...
- **Vue 3 Icon Libraries**:
  - **Lucide Icons** (`lucide-icons/lucide`) -- Clean, consistent, tree-shakeable. Default choice for Vue 3.
  - **Iconify** (`iconify/iconify`) -- Universal framework via `@iconify/vue`, access to 200K+ icons from all sets.
  - **Cultural Icon Override**: For ethnic/cultural projects, consider using culturally specific icon sets or custom SVG icons that match the design theme. Standard icon libraries may lack cultural specificity.
